...
export function usePluginCollection(pluginSlug: string, collection: string, crudUrl?: string) {
	...
	const listUrl = crudUrl || ENDPOINTS.plugins.collections.list(pluginSlug, collection);
	const itemUrl = (id: string) =>
		crudUrl ? `${crudUrl}/${id}` : ENDPOINTS.plugins.collections.item(pluginSlug, collection, id);
