...
export default function RemnawaveSquadsExternal() {
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      apiPost(`/api/remnawave/squads/external/${panelID}`, {
        name,
        description,
      }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ pid, uuid }: { pid: string; uuid: string }) =>
      apiPut(`/api/remnawave/squads/external/${pid}/${uuid}`, {
        uuid,
        name,
        description,
      }),
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-16">
        ...
        <p className="text-[13px] text-destructive">
          Failed to load external squads
        </p>
      </div>
    );
  }
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <div className="flex gap-3 pt-4">
            <button
              ...
              disabled={isSaving || !panelID || !name}
