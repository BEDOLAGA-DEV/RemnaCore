...
type InboundEntry = { uuid: string; tag: string; type: string; profileUuid?: string };
...
const FP_OPTIONS = ["", "chrome", "firefox", "safari", "ios", "android", "edge", "random", "randomized"] as const;
...
export default function RemnawaveHosts() {
	...
	const createMutation = useMutation({
		mutationFn: () => {
			...
			return apiPost(`/api/remnawave/hosts/${panelID}`, {
				...
				remark, address, port, sni, host: hostField, path,
				securityLayer, alpn: alpn || null, fingerprint: fingerprint || null,
				isDisabled,
			});
		},
	...
	const updateMutation = useMutation({
		mutationFn: (uuid: string) =>
			apiPatch(`/api/remnawave/hosts/${panelID}`, {
				uuid, remark, address, port, sni, host: hostField, path,
				securityLayer, alpn: alpn || null, fingerprint: fingerprint || null,
				isDisabled,
			}),
		onSuccess: () => { queryClient.invalidateQueries({ queryKey: QUERY_KEY }); closeForm(); },
	...
	function closeForm() {
		setFormMode("closed"); setEditingItem(null);
		setPanelID(""); setInboundUUID(""); setRemark(""); setAddress("");
		setPort(443); setSni(""); setHostField(""); setPath("");
		setSecurityLayer("DEFAULT"); setAlpn(""); setFingerprint(""); setIsDisabled(false);
	}

	function openCreate() { closeForm(); setFormMode("create"); }

	function openEdit(entry: HostEntry) {
		setFormMode("edit"); setEditingItem(entry); setPanelID(entry.panel_id);
		setRemark(entry.host.remark); setAddress(entry.host.address); setPort(entry.host.port);
		setSni(entry.host.sni); setHostField(entry.host.host); setPath(entry.host.path);
		setSecurityLayer(entry.host.securityLayer || "DEFAULT");
		setAlpn(entry.host.alpn || ""); setFingerprint(entry.host.fingerprint || "");
	...
	const ic = "w-full rounded-lg border border-border bg-background px-3 py-2 text-[13px] text-foreground placeholder:text-muted-foreground/50 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary disabled:opacity-50";
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="mt-0.5 font-mono text-[11px] text-muted-foreground">{list.length} hosts</p>
					</div>
				</div>
				...
				<div className="rounded-xl border border-border bg-card p-6">
					...
					<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
						...
						<div>
							...
							<select value={panelID} onChange={(e) => { setPanelID(e.target.value); setInboundUUID(""); }} disabled={formMode === "edit"} className={ic}>
								<option value="">Select panel...</option>
								{panels.map((p) => <option key={p.id} value={p.id}>{p.slug || p.id} — {p.url}</option>)}
							</select>
						</div>
						...
						{formMode === "create" && (
							<div>
								<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Inbound</label>
								<select value={inboundUUID} onChange={(e) => setInboundUUID(e.target.value)} className={ic}>
									<option value="">{panelID ? (inbounds.length ? "Select inbound..." : "Loading...") : "Select panel first"}</option>
									{inbounds.map((ib) => <option key={ib.uuid} value={ib.uuid}>{ib.tag} ({ib.type})</option>)}
								</select>
							</div>
						)}
						...
						{/* Remark */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Remark</label>
							<input type="text" value={remark} onChange={(e) => setRemark(e.target.value.slice(0, REMARK_MAX))} maxLength={REMARK_MAX} placeholder="Host name" className={ic} />
						</div>

						{/* Address */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Address</label>
							<input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="example.com" className={ic} />
						</div>

						{/* Port */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Port</label>
							<input type="number" value={port} onChange={(e) => setPort(Number(e.target.value))} className={ic} />
						</div>

						{/* SNI */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">SNI</label>
							<input type="text" value={sni} onChange={(e) => setSni(e.target.value)} placeholder="sni.example.com" className={ic} />
						</div>

						{/* Host */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Host</label>
							<input type="text" value={hostField} onChange={(e) => setHostField(e.target.value)} className={ic} />
						</div>

						{/* Path */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Path</label>
							<input type="text" value={path} onChange={(e) => setPath(e.target.value)} placeholder="/" className={ic} />
						</div>

						{/* Security */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Security Layer</label>
							<select value={securityLayer} onChange={(e) => setSecurityLayer(e.target.value)} className={ic}>
						...
						{/* ALPN */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">ALPN</label>
							<select value={alpn} onChange={(e) => setAlpn(e.target.value)} className={ic}>
								{ALPN_OPTIONS.map((o) => <option key={o} value={o}>{o || "— none —"}</option>)}
							</select>
						</div>

						{/* Fingerprint */}
						<div>
							<label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Fingerprint</label>
							<select value={fingerprint} onChange={(e) => setFingerprint(e.target.value)} className={ic}>
								{FP_OPTIONS.map((o) => <option key={o} value={o}>{o || "— none —"}</option>)}
							</select>
						</div>

						{/* Disabled */}
						<div className="flex items-center gap-3 pt-5">
							<button type="button" role="switch" aria-checked={isDisabled} onClick={() => setIsDisabled(!isDisabled)}
								className={cn("relative h-5 w-9 rounded-full transition-colors", isDisabled ? "bg-destructive" : "bg-muted")}>
								<span className={cn("absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-background transition-transform", isDisabled && "translate-x-4")} />
							</button>
					...
					<div className="flex gap-3 pt-4">
						<button type="button" onClick={handleSubmit}
							disabled={isSaving || !panelID || !remark || !address || (formMode === "create" && !inboundUUID)}
			...
			{deleteTarget && (
				<div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4">
					<p className="text-[13px]">Delete host <strong>{deleteTarget.host.remark}</strong>?</p>
					<div className="mt-3 flex gap-2">
						<button type="button" onClick={() => deleteMutation.mutate({ pid: deleteTarget.panel_id, uuid: deleteTarget.host.uuid })}
							disabled={deleteMutation.isPending} className="rounded-lg bg-destructive px-3 py-1.5 text-[12px] font-medium text-background">
					...
					<table className="w-full">
						...
						<tbody>
							{list.map((entry) => {
								const h = entry.host;
								return (
									<tr key={`${entry.panel_id}-${h.uuid}`} className="border-b border-border/50 last:border-0 hover:bg-secondary/30">
										<td className="px-4 py-3 text-[13px] font-semibold text-foreground">{h.remark || "—"}</td>
										<td className="px-4 py-3 font-mono text-[12px]">{h.address}:{h.port}</td>
										<td className="px-4 py-3 text-[12px]">{h.securityLayer || "—"}</td>
										...
										<td className="px-4 py-3">
											<span className={cn("inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium",
												h.isDisabled ? "bg-red-500/10 text-red-500" : "bg-emerald-500/10 text-emerald-500")}>
