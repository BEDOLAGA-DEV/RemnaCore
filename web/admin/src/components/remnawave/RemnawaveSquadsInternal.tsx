...
export default function RemnawaveSquadsInternal() {
  ...
  const {
    ...
    queryKey: QUERY_KEY,
    queryFn: () => apiGet<SquadEntry[]>(ENDPOINTS.remnawave.squadsInternal),
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ pid, uuid }: { pid: string; uuid: string }) =>
      apiPut(`/api/remnawave/squads/${pid}/${uuid}`, {
        uuid,
        name,
        description,
      }),
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-16">
        ...
        <p className="text-[13px] text-destructive">
          Failed to load internal squads
        </p>
      </div>
    );
  }
          ...
          <div className="flex gap-3 pt-4">
            <button
              ...
              disabled={
                ...
                !panelID ||
                !name ||
                (formMode === "create" && selectedInbounds.length === 0)
              }
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-[13px]">
                      {info?.membersCount != null
                        ? String(info.membersCount)
                        : "—"}
                    </td>
                    <td className="px-4 py-3 text-[13px]">
                      {info?.inboundsCount != null
                        ? String(info.inboundsCount)
                        : "—"}
                    </td>
