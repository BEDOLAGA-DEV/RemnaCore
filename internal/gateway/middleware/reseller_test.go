...
func TestRequireReseller(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "reseller role passes through",
			claims:         &authutil.UserClaims{UserID: "u-1", Email: "reseller@example.com", Role: ResellerRole},
			wantStatus:     http.StatusOK,
			wantNextCalled: true,
		},
		{
			name:           "admin role passes through",
			claims:         &authutil.UserClaims{UserID: "u-2", Email: "admin@example.com", Role: AdminRole},
			wantStatus:     http.StatusOK,
			wantNextCalled: true,
		},
		{
			name:           "customer role returns 403",
			claims:         &authutil.UserClaims{UserID: "u-3", Email: "user@example.com", Role: "customer"},
			wantStatus:     http.StatusForbidden,
			wantNextCalled: false,
			wantError:      "reseller access required",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, "/reseller/dashboard", nil)
			if tt.claims != nil {
			...
			RequireReseller(next).ServeHTTP(rec, req)
