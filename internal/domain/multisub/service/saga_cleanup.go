...
// ReportStaleOnStartup queries for sagas left in 'running' or 'compensating'
// status and logs them as warnings. These indicate sagas that were interrupted
// by a crash. Called once during application startup.
func (s *SagaCleanupService) ReportStaleOnStartup(ctx context.Context) {
	stale, err := s.sagaRepo.GetRunning(ctx)
	if err != nil {
		s.logger.Error("saga cleanup: failed to query stale sagas on startup",
			slog.Any("error", err),
		)
		return
	}
	...
	if len(stale) == 0 {
		s.logger.Info("saga cleanup: no stale sagas found on startup")
		return
	}

	s.logger.Warn("saga cleanup: found stale sagas from previous process lifetime",
	...
	for _, saga := range stale {
		...
		// Mark stale running/compensating sagas as failed so they are eligible
		// for cleanup and don't block new saga creation for the same correlation ID.
